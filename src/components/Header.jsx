...
const Header = () => {
    ...
    return (
        <header className={`${!active ? "min-h-screen" : ""}`}>
            <nav className='container max-sm:px-5 flex items-center justify-between w-full pt-[21px]'>
                ...
                <div className={`flex ease-in-out duration-300 ${active ? "max-lg:left-[-100%]" : "max-lg:start-0"}  max-lg:bg-white max-lg:flex-col max-lg:items-center max-lg:justify-center max-lg:fixed max-lg:w-full max-lg:h-full max-lg:top-0  items-center justify-center xl:gap-[30px] gap-[20px]`}>
                    {navLink.map((obj, index) => <a href={obj.href} className=' font-normal font-lato text-base text-black-300 hover:textStroke transition-all ease-in-out duration-300' onClick={removeNavBar}>{obj.tabText}</a>)}
            ...
            <section className='lg:pt-[100px] pt-[50px]'>
                <div className='container-xl max-sm:px-5 lg:bg-full bg-cover bg-bottom bg-no-repeat' style={{ backgroundImage: `url(${heroBgImg})` }}>
                    ...
                    <div className='container px-0 md:pt-[64px] pt-4'>
